...
const Todolist = () => {
  ...
  const fetchTasks = async () => {
    try {
      const response = await fetch('https://w18be.cyclic.app/user/tasks', {
        headers: {
  ...
  const handleUpdateTask = async (values) => {
    try {
      const response = await fetch(`https://w18be.cyclic.app/user/tasks/${editingTask._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${localStorage.getItem('accessToken')}`,
  ...
  const handleDeleteTask = async (taskId) => {
    try {
      const response = await fetch(`https://w18be.cyclic.app/user/tasks/${taskId}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('accessToken')}`,
        },
  ...
  const handleCompleteTask = async (taskId) => {
    try {
      const response = await fetch(`https://w18be.cyclic.app/user/tasks/complete/${taskId}`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('accessToken')}`,
        },
